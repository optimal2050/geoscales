...
test_that("aggregation up conserves the total", {
  out <- geo_recast(cap, gs, "atom", "country", rule = "sum")
...
test_that("disaggregation down splits proportionally to the weight", {
  ...
  out <- geo_recast(y, gs, "country", "state", rule = "sum", weight = "km2")
...
test_that("intensive values are copied down and weight-averaged up", {
  ...
  up <- geo_recast(eff, gs, "atom", "state", rule = "weighted_mean",
                   weight = "pop")
  ...
  down <- geo_recast(up, gs, "state", "atom", rule = "weighted_mean",
                     weight = "pop")
...
test_that("cross-cutting levels aggregate through the atom layer", {
  ...
  out <- geo_recast(s, gs, "state", "zone", rule = "sum", weight = "km2")
...
test_that("identifier columns are preserved as grouping columns", {
  ...
  out <- geo_recast(p, gs, "atom", "state", rule = "sum",
                    values = "capacity")
...
test_that("na_action controls partial coverage", {
  ...
  expect_warning(
    drop <- geo_recast(x2, gs, "atom", "country", rule = "sum"),
    "no code at level"
  )
  expect_equal(sum(drop$capacity), 21, tolerance = 1e-9)

  keep <- geo_recast(x2, gs, "atom", "country", rule = "sum",
                     na_action = "keep")
  expect_equal(sum(keep$capacity), 121, tolerance = 1e-9)
  expect_true(any(is.na(keep$country)))

  expect_error(
    geo_recast(x2, gs, "atom", "country", rule = "sum", na_action = "error"),
    "na_action"
  )
})
...
test_that("unknown source codes warn and are dropped", {
  ...
  expect_warning(out <- geo_recast(x, gs, "atom", "country", rule = "sum"),
...
test_that("the copy rule requires constant values", {
  ...
  out <- geo_recast(cc, gs, "atom", "country", rule = "copy")
  expect_equal(unique(out$rate), 0.07)

  cc$rate[1] <- 0.09
  expect_error(geo_recast(cc, gs, "atom", "country", rule = "copy"),
               "not constant")
})

test_that("bad levels and missing keys error clearly", {
  expect_error(geo_recast(cap, gs, "nope", "country"), "not a level")
  expect_error(geo_recast(cap, gs, "atom", "nope"), "not a level")
  expect_error(geo_recast(data.frame(v = 1), gs, "atom", "country"),
               "no column named")
})
