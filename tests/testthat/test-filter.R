...
test_that("navigation requires an explicit level", {
  ...
  expect_error(geoscale_children(gs, "nope", "N1"), "not a level")
  expect_error(geoscale_children(gs, "state", "GHOST"), "not found at level")
})
...
test_that("children and parents step one level", {
  ...
  expect_error(geoscale_children(gs, "atom", "A1"), "finest level")
  expect_error(geoscale_parents(gs, "country", "N"), "coarsest level")
})
...
test_that("geoscale_nests detects cross-cutting levels", {
...
test_that("prune_geoscale collapses to a coarser level and sums weights", {
