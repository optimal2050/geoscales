...
Draws a choropleth of `data` at `geoframe`. Requires `sf`, `ggplot2`,
and geometry attached with
...
- data:
  ...
  Optional `data.frame` with a code column named `geoframe` and the
  column named by `fill`. When `NULL`, region outlines are drawn.

- geoframe:

  Geoframe to draw. Defaults to the atom geoframe.
...
`energyRt::geo_map()` works exactly that way.
