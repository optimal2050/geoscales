...
``` r
register_geo_map(from, to, map, gs = NULL)

get_geo_map(from, to, gs = NULL)
...
Invisibly, the registry key. `get_geo_map()` returns the registered map
(or `NULL`); `list_geo_maps()` a `data.frame` of registry keys.
