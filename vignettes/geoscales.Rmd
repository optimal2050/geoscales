...
This is exactly why the atom layer exists, and why `recast_geoscale()` always
routes through it.
...
`recast_geoscale()` projects source values down to atoms and then aggregates them
...
`geoscale_autoplot()` draws the hierarchy itself as an icicle — one row per level,
...
With geometry attached via `attach_geometry_geoscale()`, `geoscale_plot()` draws a
