---
title: "Concepts"
description: "The mental model behind geoscales: atoms, geoframes as partitions (not a tree), authored region IDs, shares and weights, and the naming lattice shared with timescales."
...
*regions*. Different models — often different vintages of the same
model — pick different carvings: one nation, five grid regions,
thirty-two model regions, forty-six zones. The region **codes** are
arbitrary, the **weights** (area, population) are data, and converting
values between carvings is error-prone precisely where it matters:
totals must conserve, intensities must average correctly, and region
systems drawn by different hands rarely nest.
...
The **atom layer** is the finest partition — every other resolution is
a grouping of atoms. A **geoframe** is one named resolution: a column
over the atoms assigning each to a region code. Geoframes are ordered
...
**cross-cut**, with a region of one straddling two regions of the
other:
...
the `r32o11` code `APY` merges Andhra Pradesh with *part of*
Puducherry, so the 36-region carving does not nest inside the
32-region one. This is exactly why the atom layer exists, and why
`recast_geoscale()` always routes through it: neither direction of a
conversion ever assumes the geoframes nest.

## Region IDs are authored, and not unique across geoframes

The naming lattice's **ID rule** (see the glossary below): *time
COMPOSES node IDs from members; space AUTHORS them.* A timeslice ID
like `"d015_h00"` is assembled from its members; a region ID like
`"N1"` simply *is* the member, in its geoframe. Authored codes are
reused freely across geoframes:
...
atoms proportionally to the chosen weight, `weighted_mean` aggregates
an intensity up with atom weights. There is deliberately **no fallback
...
- **The package ships no maps and selects no boundaries.** Providers
  (`ne_geoscale()`, your own via `register_geoscale_provider()`) pass a
  point of view through and record it in `@meta`; the choice stays
  yours.
...
- **Explicit registries only.** Constructors return values; behavior
  changes only through `register_geoscale_rule()`, `register_geoscale_map()`,
...
- [Data structures](data-structures.html) — anatomy of a `Geoscale`
  object and its registries.
- [Data manipulation](data-manipulation.html) — attach, recast,
  crosswalks, backends.
